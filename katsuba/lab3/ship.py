...
class LightWeightShip(IShip):
    def __init__(self, port, ship_config: ConfigShip, fuel: float = 0.0) -> None:
        IShip.__init__(self, port, ship_config, fuel)
    ...
    def sail_to(self, port) -> bool:
        ...
        if fuel_needed <= self.fuel:
            ...
            arrived_port = self.port
            self.port = port
            port.incoming_ship(self)
            print(f"The LightWeightShip with ID {self.id} has arrived at the destination port {port.id}.")
            return arrived_port
        else:
            print(f"The LightWeightShip with ID {self.id} does not have enough fuel to reach the destination port.")
            return False

    def refuel(self, amount_of_fuel: float) -> None:
        self.fuel += amount_of_fuel
        print(f"The LightWeightShip with ID {self.id} has been refueled with {amount_of_fuel} units of fuel.")

    def load(self, container) -> bool:
        if len(self.containers) < self.configs.max_number_of_all_containers:
            self.containers.append(container)
            print(f"The LightWeightShip with ID {self.id} has loaded a container.")
            return True
        else:
            print(f"The LightWeightShip with ID {self.id} cannot load more containers; it has reached its capacity.")
            return False

    def unload(self, container) -> bool:
        if container in self.containers:
            self.containers.remove(container)
            print(f"The LightWeightShip with ID {self.id} has unloaded a container.")
            return True
        else:
            print(f"The specified container is not on the LightWeightShip with ID {self.id}.")
...
class MediumShip(IShip):
    ...
    def sail_to(self, port) -> bool:
        ...
        if fuel_needed <= self.fuel:
            ...
            self.port = port
            port.incoming_ship(self)
            print(f"The MediumShip with ID {self.id} has arrived at the destination port {port.id}.")
            return arrived_port
        else:
            print(f"The MediumShip with ID {self.id} does not have enough fuel to reach the destination port.")
            return False

    def refuel(self, amount_of_fuel: float) -> None:
        self.fuel += amount_of_fuel
        print(f"The MediumShip with ID {self.id} has been refueled with {amount_of_fuel} units of fuel.")

    def load(self, container) -> bool:
        if len(self.containers) < self.configs.max_number_of_all_containers:
            self.containers.append(container)
            print(f"The MediumShip with ID {self.id} has loaded a container.")
            return True
        else:
            print(f"The MediumShip with ID {self.id} cannot load more containers; it has reached its capacity.")
            return False

    def unload(self, container) -> bool:
        if container in self.containers:
            self.containers.remove(container)
            print(f"The MediumShip with ID {self.id} has unloaded a container.")
            return True
        else:
            print(f"The specified container is not on the MediumShip with ID {self.id}.")
...
class HeavyShip(IShip):
    ...
    def sail_to(self, port) -> bool:
        ...
        if fuel_needed <= self.fuel:
            ...
            self.port = port
            port.incoming_ship(self)
            print(f"The HeavyShip with ID {self.id} has arrived at the destination port {port.id}.")
            return arrived_port
        else:
            print(f"The HeavyShip with ID {self.id} does not have enough fuel to reach the destination port.")
            return False

    def refuel(self, amount_of_fuel: float) -> None:
        self.fuel += amount_of_fuel
        print(f"The HeavyShip with ID {self.id} has been refueled with {amount_of_fuel} units of fuel.")

    def load(self, container) -> bool:
        if len(self.containers) < self.configs.max_number_of_all_containers:
            self.containers.append(container)
            print(f"The HeavyShip with ID {self.id} has loaded a container.")
            return True
        else:
            print(f"The HeavyShip with ID {self.id} cannot load more containers; it has reached its capacity.")
            return False

    def unload(self, container) -> bool:
        if container in self.containers:
            self.containers.remove(container)
            print(f"The specified container is not on the HeavyShip with ID {self.id}.")
            return True
        else:
            print(f"The specified container is not on the HeavyShip with ID {self.id}.")
# кінець нового коду
