...
public:
    ...
    std::string name;
    int age;
    Operator* operatorPtr; // Вказівник на оператора
    Bill bill;
    double limitingAmount;
...
int main() {
    ...
    Customer customers[2]; // Масив клієнтів
    Operator operators[2]; // Масив операторів

    // Зчитуємо клієнтів з JSON і додаємо їх до масиву customers
    for (int i = 0; i < 2; i++) {
    ...
    // Зчитуємо операторів з JSON і додаємо їх до масиву operators
    for (int i = 0; i < 2; i++) {
    ...
    // Перевіряємо, чи є щонайменше два клієнта у масиві customers
